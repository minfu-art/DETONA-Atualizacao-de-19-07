...
test('lote editorial 31 inicia coordenação com foco sintático e rastreabilidade multifonte', async () => {
  const [raw, matrixRaw, preview] = await Promise.all([
    readFile(path.join(root, 'production/portuguese-editorial-batch-31.v1.json'), 'utf8'),
    readFile(path.join(root, 'sources/portuguese-aula07-aula02-coordination-editorial-matrix-batch-31.v1.json'), 'utf8'),
    readFile(path.join(root, 'previews/portuguese-editorial-batch-31.preview.md'), 'utf8'),
  ]);
  const course = JSON.parse(raw);
  const matrix = JSON.parse(matrixRaw);
  const questions = course.question_batches.flatMap(({ questions: items }) => items);
  const sourceIds = new Set(['prf_pdf_dd8c41255dc9', 'prf_pdf_2e7edd5477b7']);
  assert.equal(course.edital_map[0].subtopic_id, 'prf_2026_policial_rodoviario_federal_d01_t05_s02_coordenacao');
  assert.equal(course.metadata.editorial_source_matrix, 'sources/portuguese-aula07-aula02-coordination-editorial-matrix-batch-31.v1.json');
  assert.deepEqual(matrix.sources.map(({ source_id }) => source_id), [...sourceIds]);
  assert.deepEqual(matrix.sources.map(({ source_pages }) => source_pages), [[48, 49], [49, 58]]);
  assert.equal(matrix.items.length, 20);
  assert.equal(matrix.items.filter(({ source_id }) => source_id === 'prf_pdf_dd8c41255dc9').length, 2);
  assert.equal(matrix.items.filter(({ source_id }) => source_id === 'prf_pdf_2e7edd5477b7').length, 18);
  assert.ok(matrix.items.every(({ source_id }) => sourceIds.has(source_id)));
  assert.equal(course.microknowledges.length, 10);
  assert.equal(questions.length, 20);
  assert.equal(new Set(questions.map(({ statement }) => statement)).size, 20);
...
test('lote editorial 32 inicia subordinação com vinte matrizes consecutivas da Aula 07', async () => {
  const [raw, matrixRaw, preview] = await Promise.all([
    readFile(path.join(root, 'production/portuguese-editorial-batch-32.v1.json'), 'utf8'),
    readFile(path.join(root, 'sources/portuguese-aula07-subordination-editorial-matrix-batch-32.v1.json'), 'utf8'),
    readFile(path.join(root, 'previews/portuguese-editorial-batch-32.preview.md'), 'utf8'),
  ]);
  const course = JSON.parse(raw);
  const matrix = JSON.parse(matrixRaw);
  const questions = course.question_batches.flatMap(({ questions: items }) => items);
  assert.equal(course.edital_map[0].subtopic_id, 'prf_2026_policial_rodoviario_federal_d01_t05_s03_subordinaca');
  assert.equal(course.metadata.editorial_source_matrix, 'sources/portuguese-aula07-subordination-editorial-matrix-batch-32.v1.json');
  assert.deepEqual(matrix.source_question_range, [1, 20]);
  assert.deepEqual(matrix.source_pages, [50, 60]);
  assert.equal(matrix.source_id, 'prf_pdf_dd8c41255dc9');
  assert.equal(matrix.items.length, 20);
  assert.equal(new Set(matrix.items.map(({ source_question_number }) => source_question_number)).size, 20);
  assert.ok(matrix.items.every(({ source_id }) => source_id === 'prf_pdf_dd8c41255dc9'));
  assert.equal(course.microknowledges.length, 10);
  assert.equal(questions.length, 20);
  assert.equal(new Set(questions.map(({ statement }) => statement)).size, 20);
  assert.equal(questions.filter(({ correct_answer }) => correct_answer === 'C').length, 10);
...
test('lote editorial 33 continua subordinação com reduzidas paralelismo e palavra que', async () => {
  const [raw, matrixRaw, preview] = await Promise.all([
    readFile(path.join(root, 'production/portuguese-editorial-batch-33.v1.json'), 'utf8'),
    readFile(path.join(root, 'sources/portuguese-aula07-final-subordination-editorial-matrix-batch-33.v1.json'), 'utf8'),
    readFile(path.join(root, 'previews/portuguese-editorial-batch-33.preview.md'), 'utf8'),
  ]);
  const course = JSON.parse(raw);
  const matrix = JSON.parse(matrixRaw);
  const questions = course.question_batches.flatMap(({ questions: items }) => items);
  assert.equal(course.edital_map[0].subtopic_id, 'prf_2026_policial_rodoviario_federal_d01_t05_s03_subordinaca');
  assert.equal(course.metadata.editorial_source_matrix, 'sources/portuguese-aula07-final-subordination-editorial-matrix-batch-33.v1.json');
  assert.deepEqual(matrix.source_question_range, [21, 40]);
  assert.deepEqual(matrix.source_pages, [60, 72]);
  assert.equal(matrix.source_id, 'prf_pdf_dd8c41255dc9');
  assert.equal(matrix.items.length, 20);
  assert.equal(new Set(matrix.items.map(({ source_question_number }) => source_question_number)).size, 20);
  assert.ok(matrix.items.every(({ source_id }) => source_id === 'prf_pdf_dd8c41255dc9'));
...
test('lote editorial 34 fecha subordinação com as funções de que se e como', async () => {
  const [raw, matrixRaw, preview] = await Promise.all([
    readFile(path.join(root, 'production/portuguese-editorial-batch-34.v1.json'), 'utf8'),
    readFile(path.join(root, 'sources/portuguese-aula07-que-se-como-editorial-matrix-batch-34.v1.json'), 'utf8'),
    readFile(path.join(root, 'previews/portuguese-editorial-batch-34.preview.md'), 'utf8'),
  ]);
  const course = JSON.parse(raw);
  const matrix = JSON.parse(matrixRaw);
  const questions = course.question_batches.flatMap(({ questions: items }) => items);
  assert.equal(course.edital_map[0].subtopic_id, 'prf_2026_policial_rodoviario_federal_d01_t05_s03_subordinaca');
  assert.equal(course.metadata.editorial_source_matrix, 'sources/portuguese-aula07-que-se-como-editorial-matrix-batch-34.v1.json');
  assert.deepEqual(matrix.source_question_range, [41, 60]);
  assert.deepEqual(matrix.source_pages, [72, 83]);
  assert.equal(matrix.source_id, 'prf_pdf_dd8c41255dc9');
  assert.equal(matrix.items.length, 20);
  assert.equal(new Set(matrix.items.map(({ source_question_number }) => source_question_number)).size, 20);
  assert.ok(matrix.items.some(({ exam }) => exam.includes('Como 2 e 6')));
  assert.ok(matrix.items.every(({ source_id }) => source_id === 'prf_pdf_dd8c41255dc9'));
  assert.equal(course.microknowledges.length, 10);
  assert.equal(questions.length, 20);
  assert.equal(new Set(questions.map(({ statement }) => statement)).size, 20);
...
test('lote editorial 35 inicia pontuação com vinte matrizes de vírgula da Aula 08', async () => {
  const [raw, matrixRaw, preview] = await Promise.all([
    readFile(path.join(root, 'production/portuguese-editorial-batch-35.v1.json'), 'utf8'),
    readFile(path.join(root, 'sources/portuguese-aula08-comma-editorial-matrix-batch-35.v1.json'), 'utf8'),
    readFile(path.join(root, 'previews/portuguese-editorial-batch-35.preview.md'), 'utf8'),
  ]);
  const course = JSON.parse(raw);
  const matrix = JSON.parse(matrixRaw);
  const questions = course.question_batches.flatMap(({ questions: items }) => items);
  assert.equal(course.edital_map[0].subtopic_id, 'prf_2026_policial_rodoviario_federal_d01_t05_s04_pontuacao');
  assert.equal(course.metadata.editorial_source_matrix, 'sources/portuguese-aula08-comma-editorial-matrix-batch-35.v1.json');
  assert.deepEqual(matrix.source_question_range, [1, 20]);
  assert.deepEqual(matrix.source_pages, [50, 60]);
  ...
  assert.ok(course.sources.some(({ id }) => id === 'prf_pdf_bedbc3f890ba'));
  assert.equal(course.microknowledges.length, 10);
  assert.equal(questions.length, 20);
  assert.equal(new Set(questions.map(({ statement }) => statement)).size, 20);
  assert.equal(questions.filter(({ correct_answer }) => correct_answer === 'C').length, 10);
...
test('lote editorial 36 continua pontuação com as matrizes 21 a 40 de vírgula', async () => {
  const [raw, matrixRaw, preview] = await Promise.all([
    readFile(path.join(root, 'production/portuguese-editorial-batch-36.v1.json'), 'utf8'),
    readFile(path.join(root, 'sources/portuguese-aula08-comma-continuation-editorial-matrix-batch-36.v1.json'), 'utf8'),
    readFile(path.join(root, 'previews/portuguese-editorial-batch-36.preview.md'), 'utf8'),
  ]);
  const course = JSON.parse(raw);
  const matrix = JSON.parse(matrixRaw);
  const questions = course.question_batches.flatMap(({ questions: items }) => items);
  assert.equal(course.edital_map[0].subtopic_id, 'prf_2026_policial_rodoviario_federal_d01_t05_s04_pontuacao');
  assert.equal(course.metadata.editorial_source_matrix, 'sources/portuguese-aula08-comma-continuation-editorial-matrix-batch-36.v1.json');
  assert.deepEqual(matrix.source_question_range, [21, 40]);
  assert.deepEqual(matrix.source_pages, [60, 73]);
  assert.equal(matrix.source_id, 'prf_pdf_bedbc3f890ba');
  assert.equal(matrix.items.length, 20);
  assert.deepEqual(matrix.items.map(({ source_question_number }) => source_question_number), Array.from({ length: 20 }, (_, index) => index + 21));
  assert.ok(matrix.items.every(({ source_id }) => source_id === 'prf_pdf_bedbc3f890ba'));
  assert.ok(course.sources.some(({ id }) => id === 'prf_pdf_bedbc3f890ba'));
  assert.equal(course.microknowledges.length, 10);
  assert.equal(questions.length, 20);
