...
async function publishedRuntime() {
  ...
  const patch = JSON.parse(await readFile(new URL('../app/data/course-factory/published/pc-pe-2026-agente-patch-001.json', import.meta.url), 'utf8'));
  const service = new PublishedCoursePackageService({
    fetchImpl: async (url) => ({ ok: true, json: async () => structuredClone(String(url).includes('patch-001') ? patch : base) }),
  });
...
test('PC PE Agente carrega o banco ampliado publicado e disponível para venda', async () => {
  ...
  assert.equal(published.version, '2026.09.03.1');
  ...
  assert.equal(published.questions.length, 317);
  assert.equal(new Set(published.questions.map(({ id }) => id)).size, 317);
  assert.ok(published.questions.slice(0, 100).every(({ primary_microknowledge_id: id }) => Boolean(id)));
  assert.ok(published.questions.slice(100).every(({ source_batch: batch }) => batch === 'pcpe-reuso-interno-001'));
  ...
  assert.equal(seed.questions.length, 317);
...
test('registro estático da PC PE aponta para a versão e contagens publicadas', () => {
  ...
  assert.equal(entry.patchUrls.length, 1);
  assert.ok(entry.patchUrls[0].includes('pc-pe-2026-agente-patch-001.json'));
  assert.equal(entry.expectedQuestionCount, 317);
