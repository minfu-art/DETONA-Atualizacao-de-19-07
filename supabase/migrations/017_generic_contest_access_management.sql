-- Generaliza o gerenciamento administrativo de matrículas para concursos
-- cadastrados no Painel Central. Nenhum entitlement existente é removido.

alter table public.admin_access_audit
  drop constraint if exists admin_access_audit_contest_check;

alter table public.admin_access_audit
  drop constraint if exists admin_access_audit_contest_fk;

alter table public.admin_access_audit
  add constraint admin_access_audit_contest_fk
  foreign key (contest_id) references public.admin_contests(id) on delete restrict;
