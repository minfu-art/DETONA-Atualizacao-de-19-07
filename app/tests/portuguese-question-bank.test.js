...
const reinforcementPath = path.join(appRoot, 'data/questions/curated/detona_ineditas_portugues_reforco.json');
...
test('banco publicado de Português corresponde ao índice e preserva IDs únicos', async () => {
  const [questions, report, index] = await Promise.all([readJson(bankPath), readJson(reportPath), readJson(indexPath)]);
  assert.equal(questions.length, index.disciplinas.find((item) => item.id === 'lingua_portuguesa').quantidade);
  ...
  assert.ok(questions.length >= report.questoesNoBanco);
});
...
test('questões de Português estão comentadas, válidas e mapeadas ao edital', async () => {
  ...
  for (const question of questions) {
    ...
    assert.equal(validTopics.has(resolveSubtopicAlias(question.topicoEditalId)), true, question.id);
...
test('reforço autoral oferece 10 itens equilibrados em cada tópico solicitado', async () => {
  const questions = await readJson(reinforcementPath);
...
test('banco consolidado preserva legado sem colisão de IDs', async () => {
  ...
  assert.equal(publishedIds.size, questions.length);
  assert.ok(legacy.length > 0);
  assert.ok(questions.some((question) => question.metadata?.origemImport?.includes('questions_pc_al_port.json')));
});
...
test('índice preserva Análise de Dados e acrescenta Português', async () => {
  ...
  assert.ok(index.disciplinas.find((item) => item.id === 'analise_de_dados')?.quantidade >= 70);
  assert.ok(index.disciplinas.find((item) => item.id === 'lingua_portuguesa')?.quantidade >= 886);
