...
import { assertSettingsAction } from './core.js';
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const body = await request.json(); const action = assertSettingsAction(body.action);
    if (action === 'list_settings') {
      const { data, error } = await admin.from('platform_settings').select('key,value_type,value,updated_at');
      if (error) throw error; return reply(200, { settings: data }, origin);
    }
