export const CONTEST_CATALOG = [
  {
    id: 'pc_al_2026',
    code: 'PC AL',
    name: 'Policia Civil de Alagoas',
    role: 'Agente e Escrivao',
    color: '#7c6af5',
    accent: '#38bdf8',
    icon: 'PC',
    priceCents: 14990,
    currency: 'BRL',
    contentStatus: 'ready',
    salesStatus: 'available',
    careerArea: 'police_security',
    careerSubarea: 'civil_police',
    subtopicCount: 137,
    questionCount: 6480,
    description: 'Jornada completa com edital verticalizado, questoes e batalhas.',
  },
  {
    id: 'pc_ba_2026',
    code: 'PC BA',
    name: 'PC BA 2026 — Investigador de Polícia Civil',
    role: 'Investigador de Polícia Civil',
    color: '#24104f',
    accent: '#37d6ff',
    icon: 'PCBA',
    priceCents: 6990,
    currency: 'BRL',
    contentStatus: 'ready',
    salesStatus: 'available',
    examDate: '2026-12-06',
    careerArea: 'police_security',
    careerSubarea: 'civil_police',
    subtopicCount: 296,
    questionCount: 1267,
    description: 'Jornada do Investigador com edital verticalizado, microconhecimentos e banco inicial de questões.',
  },
  {
    id: 'pm_al_2026',
    code: 'PM AL',
    name: 'PM AL — Jornada de Resgate para Soldado',
    role: 'Soldado do Quadro de Praças',
    color: '#102f57',
    accent: '#f0bd35',
    icon: 'PMAL',
    priceCents: 1499,
    currency: 'BRL',
    contentStatus: 'ready',
    salesStatus: 'available',
    examDate: '2026-07-19',
    careerArea: 'police_security',
    careerSubarea: 'military_police',
    subtopicCount: 161,
    questionCount: 71,
    description: 'Preparação de retomada baseada no Edital nº 1 - PMAL/2026, com mapa completo e banco inicial de questões em expansão.',
    coverAsset: 'assets/courses/pm-al-2026.png',
  },
  {
    id: 'pc_pe_2026',
    code: 'PC PE',
    name: 'PC PE 2027 — Agente de Polícia',
    role: 'Agente de Polícia',
    color: '#13233f',
    accent: '#2dd4bf',
    icon: 'PCPE',
    priceCents: 2490,
    currency: 'BRL',
    contentStatus: 'ready',
    salesStatus: 'available',
    careerArea: 'police_security',
    careerSubarea: 'civil_police',
    subtopicCount: 188,
    questionCount: 1318,
    description: 'Jornada pré-edital 2027 com currículo verticalizado e 1.318 questões em banco ativo, sujeita à reconciliação quando sair o novo edital.',
  },
  {
    id: 'pm_pe_2027',
    code: 'PM PE',
    name: 'PM PE 2027 — Soldado',
    role: 'Praça/Soldado',
    color: '#151229',
    accent: '#ff7a00',
    icon: 'PMPE',
    priceCents: 2490,
    currency: 'BRL',
    contentStatus: 'ready',
    salesStatus: 'available',
    careerArea: 'police_security',
    careerSubarea: 'military_police',
    subtopicCount: 77,
    questionCount: 503,
    description: 'Jornada pré-edital baseada nas seis áreas do último edital oficial, com 503 questões no banco inicial conservador.',
  },
  {
    id: 'pp_pe_2027',
    code: 'PP PE',
    name: 'Polícia Penal PE 2027 — Policial Penal',
    role: 'Policial Penal',
    color: '#111827',
    accent: '#8b5cf6',
    icon: 'PPPE',
    priceCents: 2490,
    currency: 'BRL',
    contentStatus: 'ready',
    salesStatus: 'available',
    careerArea: 'police_security',
    careerSubarea: 'prison_police',
    subtopicCount: 362,
    questionCount: 444,
    description: 'Jornada pré-edital baseada no último edital oficial, com 444 questões no banco inicial conservador.',
  },
  {
    id: 'pm_ba_2026',
    code: 'PM BA',
    name: 'PM BA 2026 — Soldado',
    role: 'Aluno Soldado da Polícia Militar',
    color: '#102d25',
    accent: '#d6b34c',
    icon: 'PMBA',
    priceCents: 6999,
    currency: 'BRL',
    contentStatus: 'preparing',
    salesStatus: 'preorder',
    careerArea: 'police_security',
    careerSubarea: 'military_police',
    subtopicCount: 213,
    questionCount: 0,
    description: 'Pré-venda da jornada de Soldado da PM BA, com mapa pré-edital em preparação e acesso garantido após a publicação do conteúdo inicial.',
    coverAsset: 'assets/courses/pm-ba-2026.webp',
  },
  {
    id: 'pf_2026',
    code: 'PF',
    name: 'Policia Federal',
    role: 'Agente de Policia Federal',
    color: '#0f766e',
    accent: '#5eead4',
    icon: 'PF',
    priceCents: 18990,
    currency: 'BRL',
    contentStatus: 'preparing',
    salesStatus: 'coming_soon',
    careerArea: 'police_security',
    careerSubarea: 'federal_police',
    description: 'Modulo independente preparado para o proximo pacote editorial.',
  },
  {
    id: 'prf_2026',
    code: 'PRF',
    name: 'Policia Rodoviaria Federal',
    role: 'Policial Rodoviario Federal',
    color: '#b45309',
    accent: '#fbbf24',
    icon: 'PRF',
    priceCents: 17990,
    currency: 'BRL',
    contentStatus: 'preparing',
    salesStatus: 'coming_soon',
    careerArea: 'police_security',
    careerSubarea: 'federal_highway_police',
    description: 'Modulo independente pronto para receber edital e banco de questoes.',
  },
];

export function getContestById(contestId) {
  return CONTEST_CATALOG.find((contest) => contest.id === contestId) || null;
}

export function formatContestPrice(contest) {
  return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: contest.currency })
    .format(contest.priceCents / 100);
}
